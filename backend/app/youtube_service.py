...
def _configure_ffmpeg():
    ...
    possible_paths = [
        os.path.join(os.environ.get('TEMP', ''), 'ffmpeg', 'ffmpeg-8.0.1-essentials_build', 'bin', 'ffmpeg.exe'),
        r'C:\ffmpeg\bin\ffmpeg.exe',
        which('ffmpeg')  # Check if it's in PATH
    ]
    ...
    async def _transcribe_with_local_whisper(self, audio_file_path: str) -> Optional[str]:
        ...
        try:
            ...
            if self.whisper_model is None or self.whisper_processor is None:
                logger.error("Local Whisper model not loaded, using fallback transcript")
            ...
            if file_size > 100 * 1024 * 1024:  # 100MB limit for local processing
                logger.error(f"Audio file too large for local processing: {file_size} bytes")
                return self._generate_fallback_transcript(audio_file_path)
            
            logger.info("Starting local Whisper transcription...")
            
            # Convert M4A/other formats to WAV first if needed
            wav_file_path = audio_file_path
            if not audio_file_path.endswith('.wav'):
                try:
                    logger.info(f"Converting {os.path.splitext(audio_file_path)[1]} to WAV...")
                    audio = AudioSegment.from_file(audio_file_path)
                    wav_file_path = audio_file_path.rsplit('.', 1)[0] + '_converted.wav'
                    audio.export(wav_file_path, format='wav')
                    logger.info(f"Converted to WAV: {wav_file_path}")
                except Exception as conv_error:
                    logger.error(f"Failed to convert audio: {conv_error}")
                    logger.error("This usually means ffmpeg is not installed")
                    return self._generate_fallback_transcript(audio_file_path)
            
            # Load audio file with better error handling
            try:
                audio_array, sample_rate = librosa.load(wav_file_path, sr=16000)
                ...
                logger.error("This usually means the audio format is not supported")
                return self._generate_fallback_transcript(audio_file_path)
